...
import { getPopular, searchGamesFiltered, getDetail } from "./rawg.js"
import { useTheme } from "./ThemeContext.jsx"
import React, { useState, useMemo, useCallback, useEffect, useRef, lazy, Suspense } from "react";
import {
  useWindowWidth, strToHue, gameBg, gameAccent, toSlug,
  setMeta, DEFAULT_TITLE, DEFAULT_DESC,
  GAMES, GAME_MAP,
  Badge, Stars, MiniCover, SpoilerText,
} from "./shared.jsx";
    ...
    <div style={{ position:"fixed",bottom:mobile?90:24,right:16,zIndex:999,
      ...
      {toasts.map(t => (
        <div key={t.id} style={{
          ...
          border:`1px solid ${t.type==="error"?"#3D1515":t.type==="info"?"var(--border)":"var(--success-bg)"}`,
          color:t.type==="error"?"var(--error)":t.type==="info"?"var(--text-primary)":"var(--success)",
...
function Card({ game, ug, onOpen, onQuickAdd }) {
  ...
  const metascoreColor = game.metascore >= 85 ? 'var(--success)' : game.metascore >= 70 ? 'var(--warning)' : 'var(--error)';
              ...
              {["Want to Play","Playing","Played"].map(s=>(
                <button key={s}
                  ...
                  style={{ padding:"7px 10px",borderRadius:7,fontSize:11,fontWeight:700,
                    ...
                    background:s==="Played"?"var(--success-bg)":s==="Playing"?"#1e3a5f":"#1C1A0A",
                    color:s==="Played"?"var(--success)":s==="Playing"?"var(--primary)":"var(--accent)",
        ...
        <div style={{ overflowY:"auto",padding:mobile?"20px 20px 32px":"20px 24px 28px",flex:1 }}>
            ...
            {g.tags?.length>0 && (
              <div>
                ...
                <div style={{ display:"flex",flexWrap:"wrap",gap:5 }}>
                  {g.tags.map(t=>(
                    <span key={t} style={{ fontSize:10,color:"var(--accent)",background:"var(--accent-bg)",
                      border:"1px solid var(--accent-border)",borderRadius:4,padding:"2px 7px" }}>#{t}</span>
                  ))}
                </div>
              </div>
            )}
          ...
          <div style={{ display:"flex",gap:8,alignItems:"center",flexWrap:"wrap" }}>
            <button onClick={handleSaveWithToast} style={{ flex:mobile?1:undefined,
              ...
              background:saved?"var(--success-bg)":accent,color:saved?"var(--success)":"#000",
            ...
            {saved && !confirmDelete && (
              <button onClick={handleDeleteWithToast} style={{ padding:"11px 16px",borderRadius:8,
                background:"#1C0A0A",color:"var(--error)",
            ...
            {saved && confirmDelete && (
              <div style={{ display:"flex",gap:6,alignItems:"center" }}>
                <span style={{ fontSize:12,color:"var(--error)",fontWeight:600 }}>Are you sure?</span>
    ...
    <div style={{ display:"flex",flexDirection:wide?"column":"row",
      ...
      {activeFilters>0 && (
        <button onClick={()=>{ setPlatform(""); setYearFrom(""); setYearTo(""); }}
          style={{ padding:"6px 12px",borderRadius:6,background:"var(--border)",color:"var(--error)",
              ...
              {GENRES.map(g=>(
                <button key={g.slug} onClick={()=>setGenre(g.slug)}
                  style={{ padding:"7px 12px",borderRadius:7,fontSize:12,fontWeight:genre===g.slug?700:500,
                    ...
                    border:`1px solid ${genre===g.slug?"var(--accent)":"transparent"}`,
                    background:genre===g.slug?"var(--accent-bg)":"none",
                    color:genre===g.slug?"var(--accent)":"var(--text-tertiary)",
            ...
            {GENRES.map(g=>(
              <button key={g.slug} onClick={()=>setGenre(g.slug)}
                style={{ padding:mobile?"8px 16px":"4px 12px",borderRadius:mobile?24:20,fontSize:mobile?12:11,fontWeight:600,
                  ...
                  border:`1px solid ${genre===g.slug?"var(--accent)":"var(--border)"}`,
                  background:genre===g.slug?"var(--accent-bg)":"var(--bg-secondary)",
                  color:genre===g.slug?"var(--accent)":"var(--text-tertiary)",
...
function ProfileOverview({ games, userGames, onOpen, favorites, setFavorites }) {
  ...
  return (
    <div>
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14 }}>
          ...
          {played.length>0 && (
            <button onClick={()=>setPickingFav(!pickingFav)}
              style={{ fontSize:11,color:pickingFav?"var(--accent)":"var(--text-tertiary)",background:"none",
        ...
        {favorites.length===0 && !pickingFav ? (
          <div style={{ fontSize:13,color:"var(--text-muted)",fontStyle:"italic" }}>
            {played.length===0
              ...
              : <span>Click <b style={{color:"var(--text-tertiary)"}}>Edit</b> to pick up to 4 favorite games</span>}
          </div>
        ) : (
          <div style={{ display:"flex",gap:12,flexWrap:"wrap" }}>
                ...
                <div key={id} onClick={()=>pickingFav?toggleFav(id):onOpen(game)}
                  ...
                  {pickingFav && (
                    <div style={{ position:"absolute",top:-5,right:-5,
                      width:18,height:18,borderRadius:"50%",background:"var(--error)",
            ...
            {Array.from({length:4-favorites.length}).map((_,i)=>(
              <div key={"empty"+i}
                ...
                style={{ width:72,height:97,borderRadius:6,border:"1px dashed var(--border-hover)",
                  display:"flex",alignItems:"center",justifyContent:"center",
                  color:"var(--border-hover)",fontSize:20,cursor:"pointer" }}>+</div>
            ))}
          </div>
        )}
        {/* Pick panel */}
        {pickingFav && played.length>0 && (
          <div style={{ marginTop:14,background:"var(--bg-secondary)",borderRadius:10,
            border:"1px solid var(--border)",padding:14,maxHeight:220,overflowY:"auto" }}>
                ...
                return (
                  <button key={game.id} onClick={()=>toggleFav(game.id)}
                    style={{ padding:"4px 10px",borderRadius:6,fontSize:11,fontWeight:600,
                      ...
                      background:sel?"var(--accent-bg)":"var(--bg-tertiary)",
                      color:sel?"var(--accent)":"var(--text-secondary)" }}>{game.title}</button>
                );
              })}
            </div>
          </div>
      ...
      <div>
        <div style={{ fontSize:10,fontWeight:700,letterSpacing:"0.12em",
          color:"var(--text-muted)",textTransform:"uppercase",marginBottom:14 }}>Recent Activity</div>
      ...
      <div style={{ display:"flex",gap:6,marginBottom:18,flexWrap:"wrap" }}>
        {filters.map(f=>(
          <button key={f} onClick={()=>setFilter(f)} style={{ padding:"4px 11px",
            ...
            border:`1px solid ${filter===f?"var(--accent)":"var(--border)"}`,
            background:filter===f?"var(--accent-bg)":"var(--bg-secondary)",
            color:filter===f?"var(--accent)":"var(--text-tertiary)" }}>{f} ({counts[f]})</button>
        ))}
      </div>
      {list.length===0
...
function ProfileReviews({ games, userGames, onOpen }) {
  const list=useMemo(()=>
    games.filter(g=>userGames[g.id]?.review)
      ...
      .sort((a,b)=>new Date(b.date)-new Date(a.date)),
    [games,userGames]);
  if (list.length===0) return <div style={{ fontSize:13,color:"var(--text-muted)",fontStyle:"italic",padding:"20px 0" }}>No reviews written yet</div>;
  return (
        ...
        <div key={game.id} onClick={()=>onOpen(game)}
          ...
          <div style={{ flex:1,minWidth:0 }}>
            <div style={{ display:"flex",justifyContent:"space-between",gap:8,marginBottom:4 }}>
              <div style={{ fontWeight:700,color:"var(--text-primary)",fontSize:13 }}>{game.title}</div>
              <Badge status={status}/>
            </div>
            {rating>0 && <div style={{ marginBottom:6 }}><Stars value={rating} readonly size={12}/></div>}
            <div style={{ display:"flex",gap:8,marginBottom:6,flexWrap:"wrap" }}>
              {platform && <span style={{ fontSize:10,color:"var(--text-tertiary)",background:"var(--border)",borderRadius:4,padding:"2px 7px" }}>{platform}</span>}
              ...
              {time_played>0 && <span style={{ fontSize:10,color:"var(--text-tertiary)" }}>{time_played}h</span>}
            </div>
            {spoiler ? <SpoilerText text={review}/> : <div style={{ fontSize:12,color:"var(--text-secondary)",lineHeight:1.6 }}>{review}</div>}
            {tags?.length>0 && (
              <div style={{ display:"flex",gap:4,marginTop:6,flexWrap:"wrap" }}>
                {tags.map(t=><span key={t} style={{ fontSize:10,color:"var(--accent)",background:"var(--accent-bg)",
                  borderRadius:4,padding:"1px 6px" }}>#{t}</span>)}
              </div>
            )}
            {date && <div style={{ fontSize:10,color:"var(--border-hover)",marginTop:6 }}>
...
function ProfileWatchlist({ games, userGames, onOpen }) {
  ...
  if (list.length===0) return <div style={{ fontSize:13,color:"var(--text-muted)",fontStyle:"italic",padding:"20px 0" }}>No games in your backlog</div>;
          ...
          <div key={game.id} style={{ position:"relative" }}>
            ...
            <button onClick={e=>{ e.stopPropagation(); setPriority(game.id, targetPriority); }}
              style={{ position:"absolute",top:6,right:6,fontSize:9,fontWeight:700,padding:"2px 6px",
                ...
                color:targetPriority==="soon"?"var(--primary)":"var(--success)" }}>
...
function ProfileStats({ games, userGames }) {
  ...
  if (stats.played===0) return (
    <div style={{ fontSize:13,color:"var(--text-muted)",fontStyle:"italic",padding:"20px 0" }}>
      Log some games to see stats
      <button onClick={exportCSV} style={{ display:"block",marginTop:12,padding:"8px 14px",borderRadius:7,
        background:"var(--bg-secondary)",border:"1px solid var(--border)",color:"var(--text-tertiary)",fontSize:11,cursor:"pointer" }}>
        Export library (CSV)
      </button>
    </div>
  );
    ...
    <div>
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:24 }}>
        {[
          ...
          <div key={s.label} style={{ background:"var(--bg-secondary)",border:"1px solid var(--border)",
            borderRadius:10,padding:"12px 14px",textAlign:"center" }}>
            <div style={{ fontSize:22,fontWeight:900,color:"var(--accent)",lineHeight:1 }}>{s.value}</div>
            <div style={{ fontSize:9,color:"var(--text-muted)",marginTop:4,fontWeight:700,
        ...
        <div style={{ display:"flex",gap:3,alignItems:"flex-end",height:72 }}>
          {stats.ratingDist.map(({ r, count })=>(
            <div key={r} style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2 }}>
              {count>0 && <div style={{ fontSize:9,color:"var(--text-tertiary)" }}>{count}</div>}
              <div style={{ width:"100%",borderRadius:2,background:"var(--accent)",
            ...
            {stats.topGenres.map(([genre,count],i)=>{
              ...
              return (
                <div key={genre} style={{ display:"flex",alignItems:"center",gap:12 }}>
                  <div style={{ width:90,fontSize:11,color:"var(--text-secondary)",textAlign:"right",flexShrink:0 }}>{genre}</div>
                  ...
                  <div style={{ fontSize:11,color:"var(--text-muted)",width:16,flexShrink:0 }}>{count}</div>
                </div>
              );
            })}
          </div>
        ...
        <div style={{ marginBottom:28 }}>
          ...
          <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
            {stats.topPlatforms.map(([plat,count])=>(
              <div key={plat} style={{ background:"var(--bg-secondary)",border:"1px solid var(--border)",
                borderRadius:8,padding:"6px 12px",textAlign:"center" }}>
                <div style={{ fontSize:13,fontWeight:800,color:"var(--text-primary)" }}>{count}</div>
                <div style={{ fontSize:9,color:"var(--text-muted)" }}>{plat}</div>
              </div>
            ))}
          </div>
        </div>
        ...
        <div style={{ marginBottom:28 }}>
          ...
          <div style={{ display:"flex",gap:8,flexWrap:"wrap" }}>
            {stats.topYears.map(([year,count])=>(
              <div key={year} style={{ background:"var(--bg-secondary)",border:"1px solid var(--border)",
                borderRadius:8,padding:"8px 14px",textAlign:"center" }}>
                <div style={{ fontSize:16,fontWeight:800,color:"var(--text-primary)" }}>{year}</div>
                <div style={{ fontSize:10,color:"var(--text-muted)" }}>{count} game{count!==1?"s":""}</div>
              </div>
            ))}
          </div>
        </div>
...
// ─── Steam Import ──────────────────────────────────────────────────
function SteamImport({ handleSave }) {
  const [apiKey,  setApiKey]  = useState(() => localStorage.getItem("steam_api_key") || "");
  ...
  const fetchLibrary = async () => {
    ...
    try {
      ...
      localStorage.setItem("steam_id",      steamId.trim());
      const { data, error: fnErr } = await supabase.functions.invoke("steam-import", {
        body: { steamId: steamId.trim(), apiKey: apiKey.trim() },
      });
      if (fnErr)      throw new Error(fnErr.message);
      if (data?.error) throw new Error(data.error);
      const sorted = [...data.games].sort((a, b) => b.playtime - a.playtime);
      setGames(sorted);
      setSelected(new Set(sorted.filter(g => g.playtime > 0).map(g => g.appid)));
      setStep("preview");
    } catch (e) {
      setError(e.message);
    } finally {
  ...
  const runImport = async () => {
    ...
    for (let i = 0; i < toImport.length; i++) {
      const sg = toImport[i];
      try {
        const results = await searchGamesFiltered(sg.name, 1);
        if (results.length > 0) {
          const rg = results[0];
          const status = sg.playtime >= 60 ? "Played" : "Want to Play";
          await handleSave(rg.id, {
            status,
            rating: 0,
            review: "",
            ...
            year: rg.year,
            genre: rg.genre,
            developer: rg.developer,
            time_played: sg.playtime >= 60 ? Math.round(sg.playtime / 60) : null,
          });
        } else {
          sk++;
        }
  ...
  const toggle = (appid) =>
    setSelected(prev => { const s = new Set(prev); s.has(appid) ? s.delete(appid) : s.add(appid); return s; });

  const fmtHrs = (mins) => mins >= 60 ? `${Math.round(mins/60)}h` : `${mins}m`;

  if (step === "form") return (
    <div style={{ maxWidth:480 }}>
      <div style={{ fontSize:18,fontWeight:800,color:"var(--text-primary)",marginBottom:6 }}>Import from Steam</div>
      <div style={{ fontSize:13,color:"var(--text-secondary)",lineHeight:1.6,marginBottom:20 }}>
        Pulls your Steam library and adds games to your Backlog. Requires a free Steam Web API key and your profile visibility set to <b>Public</b>.
      </div>
      <div style={{ marginBottom:14 }}>
        <div style={{ fontSize:11,fontWeight:700,color:"var(--text-muted)",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6 }}>
          ...
          <a href="https://steamcommunity.com/dev/apikey" target="_blank" rel="noopener noreferrer"
            style={{ marginLeft:8,color:"var(--accent)",fontWeight:600,textTransform:"none",fontSize:11 }}>
            Get one free →
          </a>
        </div>
      ...
      <div style={{ marginBottom:20 }}>
        <div style={{ fontSize:11,fontWeight:700,color:"var(--text-muted)",letterSpacing:"0.1em",textTransform:"uppercase",marginBottom:6 }}>
          Steam Vanity URL or Steam64 ID
        </div>
        <input value={steamId} onChange={e=>setSteamId(e.target.value)}
          placeholder="yourvanityurl  or  76561198012345678"
          style={{ width:"100%",padding:"10px 12px",background:"var(--bg-tertiary)",
            border:"1px solid var(--border)",borderRadius:8,color:"var(--text-primary)",
            fontSize:13,boxSizing:"border-box" }}/>
        <div style={{ fontSize:11,color:"var(--text-muted)",marginTop:5 }}>
          Your vanity URL is the last part of your Steam profile URL: steamcommunity.com/id/<b>yourvanityurl</b>
        </div>
      </div>
      {error && <div style={{ fontSize:12,color:"var(--error)",marginBottom:14,padding:"8px 12px",
        background:"#1C0A0A",border:"1px solid #3D1515",borderRadius:7 }}>{error}</div>}
      <button onClick={fetchLibrary} disabled={busy || !apiKey.trim() || !steamId.trim()}
        style={{ padding:"11px 24px",borderRadius:9,background:"var(--accent)",color:"#000",
          border:"none",fontWeight:800,fontSize:14,cursor:"pointer",opacity:(busy||!apiKey||!steamId)?0.5:1 }}>
        {busy ? "Fetching library…" : "Fetch Steam Library"}
      </button>
    </div>
  );

  if (step === "preview") {
    const played  = games.filter(g => g.playtime >= 60);
    const backlog = games.filter(g => g.playtime > 0 && g.playtime < 60);
    ...
    return (
      <div>
        ...
        <div style={{ fontSize:13,color:"var(--text-secondary)",marginBottom:16 }}>
          {played.length} played (1h+) will be added as <b>Played</b>. {backlog.length} with some playtime as <b>Want to Play</b>. Uncheck any you want to skip.
        </div>
        <div style={{ display:"flex",gap:8,marginBottom:14 }}>
          <button onClick={()=>setSelected(new Set(games.map(g=>g.appid)))}
            style={{ padding:"5px 12px",borderRadius:6,background:"var(--bg-tertiary)",border:"1px solid var(--border)",
              color:"var(--text-secondary)",fontSize:11,cursor:"pointer" }}>Select all</button>
          <button onClick={()=>setSelected(new Set(played.map(g=>g.appid)))}
            style={{ padding:"5px 12px",borderRadius:6,background:"var(--bg-tertiary)",border:"1px solid var(--border)",
              color:"var(--text-secondary)",fontSize:11,cursor:"pointer" }}>Played only</button>
          <button onClick={()=>setSelected(new Set())}
            style={{ padding:"5px 12px",borderRadius:6,background:"var(--bg-tertiary)",border:"1px solid var(--border)",
              color:"var(--text-secondary)",fontSize:11,cursor:"pointer" }}>Deselect all</button>
        </div>
        <div style={{ maxHeight:420,overflowY:"auto",border:"1px solid var(--border)",borderRadius:10,marginBottom:16 }}>
          {[["Played (1h+)", played], ["Some playtime", backlog], ["Unplayed", unplayed]].map(([label, list]) =>
            list.length === 0 ? null : (
              <div key={label}>
                <div style={{ padding:"8px 14px",fontSize:10,fontWeight:700,color:"var(--text-muted)",
                  letterSpacing:"0.1em",textTransform:"uppercase",background:"var(--bg-tertiary)",
                  borderBottom:"1px solid var(--border)" }}>{label} ({list.length})</div>
                {list.map((g, i) => (
                  <div key={g.appid} onClick={()=>toggle(g.appid)}
                    style={{ display:"flex",alignItems:"center",gap:10,padding:"9px 14px",cursor:"pointer",
                      borderBottom: i < list.length-1 ? "1px solid var(--border)" : "none",
                      background:selected.has(g.appid) ? "var(--accent-bg)" : "transparent",
                    ...
                    {g.icon && <img src={g.icon} alt="" style={{ width:28,height:28,borderRadius:4,flexShrink:0 }}/>}
                    ...
                    {g.playtime > 0 && (
                      <span style={{ fontSize:11,color:"var(--text-muted)",flexShrink:0 }}>{fmtHrs(g.playtime)}</span>
                    )}
                  </div>
                ))}
              </div>
        ...
        <button onClick={runImport} disabled={selected.size === 0}
          style={{ padding:"11px 24px",borderRadius:9,background:"var(--accent)",color:"#000",
            border:"none",fontWeight:800,fontSize:14,cursor:"pointer",opacity:selected.size===0?0.5:1 }}>
  ...
  if (step === "importing") return (
    <div style={{ maxWidth:400 }}>
      ...
      <div style={{ background:"var(--bg-tertiary)",borderRadius:8,height:8,overflow:"hidden",marginBottom:10 }}>
        <div style={{ height:"100%",background:"var(--accent)",borderRadius:8,
          width:`${progress.total ? (progress.done/progress.total)*100 : 0}%`,
          transition:"width 0.3s ease" }}/>
      </div>
      <div style={{ fontSize:13,color:"var(--text-secondary)" }}>
        {progress.done} / {progress.total} games processed
      </div>
    </div>
  );
  ...
  return (
    <div style={{ maxWidth:400 }}>
      ...
      <div style={{ fontSize:20,fontWeight:800,color:"var(--text-primary)",marginBottom:8 }}>Import complete</div>
      <div style={{ fontSize:14,color:"var(--text-secondary)",lineHeight:1.7 }}>
        <b>{progress.total - skipped}</b> games added to your library.
        {skipped > 0 && <> <b>{skipped}</b> could not be matched to RAWG and were skipped.</>}
      </div>
      <button onClick={()=>{ setStep("form"); setGames([]); setSkipped(0); }}
        style={{ marginTop:20,padding:"10px 20px",borderRadius:8,background:"var(--bg-tertiary)",
...
function Profile({ games, userGames, onOpen, favorites, setFavorites, profile, setProfile, session, handleSave }) {
  ...
  return (
    <div>
                    ...
                    <div style={{ display:"flex",alignItems:"center",gap:10,marginTop:10,marginBottom:2 }}>
                      ...
                      <button onClick={()=>setDraftPublic(p=>!p)}
                        style={{ padding:"4px 12px",borderRadius:20,fontSize:11,fontWeight:700,cursor:"pointer",
                          border:`1px solid ${draftPublic?"var(--success)":"var(--border-hover)"}`,
                          background:draftPublic?"var(--success-bg)":"var(--bg-tertiary)",
                          color:draftPublic?"var(--success)":"var(--text-tertiary)" }}>
                  ...
                  <div>
                    ...
                    <div style={{ display:"flex",gap:6,flexWrap:"wrap",alignItems:"center" }}>
                      ...
                      <span style={{ fontSize:10,fontWeight:700,padding:"3px 10px",borderRadius:5,
                        border:`1px solid ${profile.isPublic?"var(--success)":"var(--border-hover)"}`,
                        color:profile.isPublic?"var(--success)":"var(--text-muted)",letterSpacing:"0.05em" }}>
                        {profile.isPublic?"PUBLIC":"PRIVATE"}
                      </span>
                      {profile.isPublic && (
                        <button onClick={copyLink}
                          style={{ fontSize:10,fontWeight:700,padding:"3px 10px",borderRadius:5,
                            background:copied?"var(--success-bg)":"none",
                            border:`1px solid ${copied?"var(--success)":"var(--border-hover)"}`,
                            color:copied?"var(--success)":"var(--text-tertiary)",cursor:"pointer",letterSpacing:"0.05em" }}>
                          {copied?"COPIED!":"SHARE PROFILE"}
                        </button>
                      )}
                      <button onClick={()=>setSubTab("import")}
                        style={{ fontSize:10,fontWeight:700,padding:"3px 10px",borderRadius:5,
                          background:subTab==="import"?"var(--accent-bg)":"none",
                          border:`1px solid ${subTab==="import"?"var(--accent)":"var(--border-hover)"}`,
                          color:subTab==="import"?"var(--accent)":"var(--text-tertiary)",
      ...
      <div style={{ padding:mobile?"20px 16px 80px":"28px 20px 56px",maxWidth:760,margin:"0 auto" }}>
        ...
        {subTab==="import"       && <SteamImport handleSave={handleSave}/>}
      </div>
    </div>
  );
}
    ...
    <div>
          ...
          {years.map(y=>(
            <button key={y} onClick={()=>setYear(y)}
              style={{ padding:"4px 12px",borderRadius:20,fontSize:11,fontWeight:700,cursor:"pointer",
                ...
                background:y===year?"var(--accent-bg)":"var(--bg-secondary)",
                color:y===year?"var(--accent)":"var(--text-tertiary)" }}>{y}</button>
          ))}
        </div>
      )}
      ...
      <div style={{ display:"grid",gridTemplateColumns:"repeat(2,1fr)",gap:10,marginBottom:24 }}>
        {[
          ...
          <div key={s.label} style={{ background:"var(--bg-secondary)",border:"1px solid var(--border)",
            borderRadius:10,padding:"12px 14px",textAlign:"center" }}>
            <div style={{ fontSize:20,fontWeight:900,color:"var(--accent)",lineHeight:1.1 }}>{s.value}</div>
            <div style={{ fontSize:9,color:"var(--text-muted)",marginTop:4,fontWeight:700,
              letterSpacing:"0.07em",textTransform:"uppercase" }}>{s.label}</div>
          </div>
        ))}
      ...
      {best && (
        <div style={{ background:"linear-gradient(135deg,#1A1025,#0D1A0D)",border:"1px solid var(--border)",
          ...
          <div>
            <div style={{ fontSize:9,fontWeight:700,color:"var(--accent)",letterSpacing:"0.12em",
              textTransform:"uppercase",marginBottom:4 }}>Your best of {year}</div>
            <div style={{ fontSize:15,fontWeight:800,color:"var(--text-primary)",marginBottom:4 }}>{best.game?.title}</div>
        ...
        <div style={{ display:"flex",gap:6,alignItems:"flex-end",height:70 }}>
          {monthCounts.map((count,i)=>(
            <div key={i} style={{ flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:3 }}>
              <div style={{ width:"100%",borderRadius:3,background:count>0?"var(--accent)":"var(--border)",
                height:`${(count/maxMonth)*50}px`,minHeight:count>0?4:0 }}/>
              <div style={{ fontSize:8,color:"var(--text-muted)" }}>{MONTHS[i].slice(0,1)}</div>
            </div>
          ))}
        </div>
      </div>
        ...
        <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
          {yearEntries.sort((a,b)=>new Date(b.date)-new Date(a.date)).map(e=>(
            <div key={e.game.id} style={{ display:"flex",gap:12,alignItems:"center",
              padding:"8px 12px",background:"var(--bg-secondary)",borderRadius:8,border:"1px solid var(--border)" }}>
...
function ProfileLists({ games, userGames, session, onOpen }) {
  ...
  if (loading) return <div style={{ fontSize:13,color:"var(--text-muted)",padding:"20px 0" }}>Loading lists...</div>;
  if (error) return (
    <div style={{ fontSize:13,color:"var(--text-muted)",fontStyle:"italic",padding:"20px 0" }}>
    ...
    return (
      <div>
        ...
        <div style={{ display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16 }}>
          <div>
            <div style={{ fontSize:16,fontWeight:800,color:"var(--text-primary)",marginBottom:4 }}>{selectedList.title}</div>
            {selectedList.description && <div style={{ fontSize:12,color:"var(--text-secondary)" }}>{selectedList.description}</div>}
          </div>
          <span style={{ fontSize:10,fontWeight:700,padding:"3px 8px",borderRadius:4,
            border:`1px solid ${selectedList.is_public?"var(--success)":"var(--border-hover)"}`,
            color:selectedList.is_public?"var(--success)":"var(--text-muted)" }}>
            {selectedList.is_public?"PUBLIC":"PRIVATE"}
          </span>
        </div>
        <div style={{ display:"flex",gap:8,marginBottom:16 }}>
          <button onClick={()=>setAddingGame(a=>!a)}
            style={{ padding:"7px 14px",borderRadius:7,background:"var(--accent)",color:"#000",
        ...
        {addingGame && (
          <div style={{ background:"var(--bg-secondary)",border:"1px solid var(--border)",borderRadius:10,
            ...
            <div style={{ fontSize:11,color:"var(--text-muted)",marginBottom:8 }}>Pick from your library:</div>
        ...
        {listGames.length===0 ? (
          <div style={{ fontSize:13,color:"var(--text-muted)",fontStyle:"italic" }}>No games in this list yet</div>
        ) : (
          <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
            {listGames.map((item,idx)=>{
              ...
              return (
                <div key={item.game_id} style={{ display:"flex",gap:12,alignItems:"center",
                  padding:"10px 12px",background:"var(--bg-secondary)",borderRadius:8,border:"1px solid var(--border)" }}>
                  <div style={{ color:"var(--text-muted)",fontSize:11,fontWeight:700,width:20,textAlign:"center" }}>{idx+1}</div>
                  <div style={{ width:36,height:48,borderRadius:4,flexShrink:0,overflow:"hidden",
                  ...
                  <button onClick={()=>removeFromList(item.game_id)}
                    style={{ background:"none",border:"none",cursor:"pointer",color:"var(--error)",fontSize:14,padding:4 }}>×</button>
                </div>
              );
            })}
          </div>
    ...
    <div>
      {creating ? (
        <div style={{ background:"var(--bg-secondary)",border:"1px solid var(--border)",borderRadius:10,padding:16,marginBottom:20 }}>
          <div style={{ fontSize:13,fontWeight:700,color:"var(--text-primary)",marginBottom:12 }}>New list</div>
          <input value={newTitle} onChange={e=>setNewTitle(e.target.value)} placeholder="List title"
            style={{ width:"100%",padding:"8px 11px",background:"var(--bg-primary)",border:"1px solid var(--border)",
              borderRadius:7,color:"var(--text-primary)",fontSize:13,outline:"none",marginBottom:8,boxSizing:"border-box" }}/>
          <input value={newDesc} onChange={e=>setNewDesc(e.target.value)} placeholder="Description (optional)"
            style={{ width:"100%",padding:"8px 11px",background:"var(--bg-primary)",border:"1px solid var(--border)",
              borderRadius:7,color:"var(--text-primary)",fontSize:13,outline:"none",marginBottom:10,boxSizing:"border-box" }}/>
          <label style={{ display:"flex",alignItems:"center",gap:7,marginBottom:12,cursor:"pointer" }}>
            ...
            <span style={{ fontSize:12,color:"var(--text-tertiary)" }}>Make list public</span>
          </label>
          <div style={{ display:"flex",gap:8 }}>
            <button onClick={createList} style={{ padding:"7px 16px",borderRadius:7,background:"var(--accent)",
            ...
            <button onClick={()=>setCreating(false)} style={{ padding:"7px 14px",borderRadius:7,
              background:"var(--border)",color:"var(--text-secondary)",border:"1px solid var(--border)",fontSize:12,cursor:"pointer" }}>Cancel</button>
          </div>
        </div>
      ) : (
        <button onClick={()=>setCreating(true)} style={{ padding:"8px 16px",borderRadius:8,
          background:"var(--accent)",color:"#000",border:"none",fontWeight:700,fontSize:12,
          cursor:"pointer",marginBottom:16 }}>+ New list</button>
      )}
      {lists.length===0 ? (
        <div style={{ fontSize:13,color:"var(--text-muted)",fontStyle:"italic" }}>No lists yet. Create one to curate your favorite games.</div>
      ) : (
        <div style={{ display:"flex",flexDirection:"column",gap:10 }}>
          {lists.map(l=>(
            <div key={l.id} style={{ background:"var(--bg-secondary)",border:"1px solid var(--border)",borderRadius:10,
              padding:"12px 14px",display:"flex",alignItems:"center",gap:12 }}>
              <div style={{ flex:1,minWidth:0,cursor:"pointer" }}
                onClick={()=>{ setSelectedList(l); loadListGames(l.id); }}>
                <div style={{ fontWeight:700,color:"var(--text-primary)",fontSize:13,marginBottom:2 }}>{l.title}</div>
                {l.description && <div style={{ fontSize:11,color:"var(--text-secondary)" }}>{l.description}</div>}
                <span style={{ fontSize:9,fontWeight:700,padding:"2px 6px",borderRadius:3,
                  border:`1px solid ${l.is_public?"var(--success)":"var(--border-hover)"}`,
                  color:l.is_public?"var(--success)":"var(--text-muted)",marginTop:4,display:"inline-block" }}>
              ...
              {deleteConfirm===l.id ? (
                <div style={{ display:"flex",gap:5,alignItems:"center" }}>
                  <button onClick={()=>deleteList(l.id)} style={{ padding:"5px 10px",borderRadius:6,
                    background:"var(--error)",color:"#fff",border:"none",fontSize:11,fontWeight:700,cursor:"pointer" }}>Delete</button>
                  <button onClick={()=>setDeleteConfirm(null)} style={{ padding:"5px 10px",borderRadius:6,
                    background:"var(--border)",color:"var(--text-secondary)",border:"1px solid var(--border)",fontSize:11,cursor:"pointer" }}>Cancel</button>
                </div>
              ) : (
  ...
  return (
    <div>
      ...
      {searched && !loading && results.length===0 && (
        <div style={{ fontSize:13,color:"var(--text-muted)",fontStyle:"italic" }}>No public users found for "{q}"</div>
      )}
      {results.length>0 && (
        <div style={{ display:"flex",flexDirection:"column",gap:8 }}>
          {results.map(u=>(
            <a key={u.username} href={`/backlog/#/u/${u.username}`}
              ...
              <div style={{ flex:1,minWidth:0 }}>
                <div style={{ fontWeight:700,color:"var(--text-primary)",fontSize:13,marginBottom:2 }}>{u.username}</div>
                {u.bio && <div style={{ fontSize:11,color:"var(--text-secondary)",whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis" }}>{u.bio}</div>}
              </div>
              <span style={{ fontSize:10,fontWeight:700,padding:"2px 8px",borderRadius:4,
                background:"var(--accent-bg)",border:"1px solid var(--accent-border)",color:"var(--accent)" }}>View →</span>
            </a>
          ))}
        </div>
      )}
  ...
  return (
    <div style={{ position:"relative" }} ref={panelRef}>
      <button onClick={()=>{ setOpen(o=>!o); if(!open) markRead(); }}
        ...
        🔔
        {unread>0 && (
          <span style={{ position:"absolute",top:-2,right:2,background:"var(--error)",color:"#fff",
      ...
      {open && (
        <div style={{ position:"absolute",right:0,top:"calc(100% + 8px)",width:280,
          ...
          {notifications.length===0 ? (
            <div style={{ padding:"20px 14px",fontSize:12,color:"var(--text-muted)",textAlign:"center",fontStyle:"italic" }}>
              No notifications yet
            </div>
          ) : (
            <div style={{ maxHeight:320,overflowY:"auto" }}>
              {notifications.map(n=>(
                <div key={n.id} style={{ padding:"10px 14px",borderBottom:"1px solid var(--bg-secondary)",
...
function ActivityFeed({ session }) {
  ...
  if (loading) return <div style={{ fontSize:13,color:"var(--text-muted)",padding:"20px 0" }}>Loading activity...</div>;
  if (error) return (
    <div style={{ fontSize:13,color:"var(--text-muted)",fontStyle:"italic",padding:"20px 0" }}>
  ...
  if (!feed.length) return (
    <div style={{ padding:"32px 0",textAlign:"center" }}>
      <div style={{ fontSize:36,marginBottom:10 }}>👥</div>
      <div style={{ fontSize:14,color:"var(--text-secondary)",fontWeight:700,marginBottom:6 }}>No activity yet</div>
      <div style={{ fontSize:12,color:"var(--text-muted)" }}>Follow other users to see their game logs here</div>
    </div>
  );
        ...
        <div key={i} style={{ display:"flex",gap:12,alignItems:"center",padding:"10px 12px",
          ...
          <div style={{ flex:1,minWidth:0 }}>
            <div style={{ fontSize:12,color:"var(--text-secondary)",marginBottom:2 }}>
              ...
              <span style={{ color:"var(--text-primary)",fontWeight:600 }}>{row.game_title}</span>
            </div>
  ...
  render() {
    ...
    return (
      <div style={{ minHeight:"100vh",background:"var(--bg-primary)",display:"flex",flexDirection:"column",
        alignItems:"center",justifyContent:"center",gap:12,fontFamily:"'Inter',system-ui,sans-serif",
        ...
        <button onClick={()=>window.location.reload()}
          style={{ marginTop:8,padding:"10px 24px",borderRadius:9,background:"var(--accent)",
...
export default function App() {
  ...
  const [searchHistory, setSearchHistory] = useState(() => {
    try { return JSON.parse(localStorage.getItem("backlog_search_history") || "[]"); } catch { return []; }
  });
  ...
  const pushHistory = useCallback((term) => {
    ...
    setSearchHistory(prev => {
      ...
      localStorage.setItem("backlog_search_history", JSON.stringify(next));
      return next;
    });
  }, []);
  ...
  const lazyfallback = <div style={{ minHeight:"100vh",background:"var(--bg-primary)" }}/>;

  if (publicUsername) return <ErrorBoundary><Suspense fallback={lazyfallback}><PublicProfile username={publicUsername}/></Suspense></ErrorBoundary>;

  if (view==="loading") return (
    <div style={{ minHeight:"100vh",background:"var(--bg-primary)",display:"flex",
      alignItems:"center",justifyContent:"center",gap:12,
      fontFamily:"'Inter',system-ui,sans-serif",color:"var(--text-tertiary)",fontSize:14 }}>
      <span style={{ fontSize:24 }}>🎮</span> Loading…
    </div>
  );

  if (view==="landing") return <ErrorBoundary><Suspense fallback={lazyfallback}><LandingPage onGoAuth={goAuth}/></Suspense></ErrorBoundary>;
  if (view==="auth")    return <ErrorBoundary><Suspense fallback={lazyfallback}><AuthPage initialMode={authMode} onAuth={handleAuth} onBack={()=>setView("landing")}/></Suspense></ErrorBoundary>;
    ...
    <div style={{ minHeight:"100vh",backgroundColor:"var(--bg-primary)",
      ...
      <nav style={{ background:"var(--bg-primary)",borderBottom:"1px solid var(--border)",
        ...
        {mobile ? (
          <>
            ...
            {showSearch && (
              <div style={{ flex:1,position:"relative" }}>
                <input value={q}
                  onChange={e=>{ setQ(e.target.value); if(e.target.value) setTab("discover"); }}
                  autoFocus
                  onFocus={()=>setSearchFocused(true)}
                  onBlur={()=>{ setTimeout(()=>setSearchFocused(false),150); if(!q) setShowSearch(false); if(q.trim()) pushHistory(q.trim()); }}
                  placeholder="Search games..."
                  style={{ width:"100%",padding:"6px 11px",background:"var(--bg-tertiary)",
                ...
                {searchFocused && !q && searchHistory.length>0 && (
                  <div style={{ position:"absolute",top:"100%",left:0,right:0,marginTop:4,
                    ...
                    borderRadius:8,zIndex:200,overflow:"hidden",boxShadow:"0 8px 24px rgba(0,0,0,0.3)" }}>
                    <div style={{ fontSize:10,fontWeight:700,color:"var(--text-muted)",
                      letterSpacing:"0.1em",padding:"8px 12px 4px",textTransform:"uppercase" }}>Recent</div>
                    {searchHistory.map(h=>(
                      <button key={h} onMouseDown={()=>{ setQ(h); setTab("discover"); pushHistory(h); }}
                        style={{ width:"100%",textAlign:"left",padding:"8px 12px",background:"none",
                          border:"none",borderTop:"1px solid var(--border)",cursor:"pointer",
                          fontSize:13,color:"var(--text-primary)" }}>{h}</button>
                    ))}
                  </div>
                )}
              </div>
          ...
          <div style={{ position:"relative",marginRight:12 }}>
            <span style={{ position:"absolute",left:9,top:"50%",transform:"translateY(-50%)",color:"var(--text-muted)",fontSize:13,zIndex:1 }}>🔍</span>
            <input value={q}
              onChange={e=>{ setQ(e.target.value); if(e.target.value) setTab("discover"); }}
              onFocus={()=>setSearchFocused(true)}
              onBlur={()=>{ setTimeout(()=>setSearchFocused(false),150); if(q.trim()) pushHistory(q.trim()); }}
              placeholder="Search..."
            ...
            {searchFocused && !q && searchHistory.length>0 && (
              <div style={{ position:"absolute",top:"100%",left:0,right:0,marginTop:4,
                ...
                {searchHistory.map(h=>(
                  <button key={h} onMouseDown={()=>{ setQ(h); setTab("discover"); pushHistory(h); }}
                    style={{ width:"100%",textAlign:"left",padding:"7px 12px",background:"none",
                      border:"none",borderTop:"1px solid var(--border)",cursor:"pointer",
                      fontSize:12,color:"var(--text-primary)" }}>{h}</button>
                ))}
              </div>
            )}
          </div>
        ...
        {!mobile && (
          <button onClick={handleLogout}
            style={{ fontSize:11,fontWeight:700,color:"var(--text-muted)",background:"none",
        ...
        {mobile && (
          <button onClick={handleLogout}
            style={{ background:"none",border:"1px solid var(--border)",cursor:"pointer",
              color:"var(--text-tertiary)",fontSize:11,fontWeight:700,flexShrink:0,
        ...
        <button onClick={toggleTheme}
          ...
          <div style={{ width:44,height:24,borderRadius:12,position:"relative",
            ...
            <div style={{ position:"absolute",top:3,
              ...
              background:theme==="dark"?"var(--text-tertiary)":"var(--warning)",
      ...
      <div
        ...
        {tab==="profile" &&<Profile  games={loggedGames} userGames={userGames} onOpen={setGame}
          ...
          session={session}    handleSave={handleSave}/>}
      </div>
